...
alias pips='f() {
  pip install "$@" --break-system-packages && {
    ...
    for pkg in "$@"; do
      [[ "$pkg" == -* ]] && continue
      local name=$(pip show "$pkg" 2>/dev/null | grep "^Name:" | cut -d" " -f2)
      local ver=$(pip show "$pkg" 2>/dev/null | grep "^Version:" | cut -d" " -f2)
      if [ -n "$name" ] && [ -n "$ver" ]; then
        local pattern=$(echo "$name" | sed "s/[-_]/[-_]/g")
        if [ -f "$req" ]; then
          if grep -iq "^${pattern}==" "$req"; then
            sed -i "" -E "s/^${pattern}==.*/${name}==${ver}/I" "$req"
          else
            echo "${name}==${ver}" >> "$req"
          fi
        else
...
# Set Airpod speaker as output + MacBook mic as input
